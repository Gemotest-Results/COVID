(function () {
  window.APP = {
    ...
    toogleLang: function () {
      ...
      var elements = document.body.querySelectorAll('[data-' + this.lang + ']');
      for (var i = 0; i < elements.length; i++) {
        elements[i].innerHTML = elements[i].getAttribute("data-" + this.lang);
      }
      this.init();
    },
    ...
    init: function () {
      ...
      unrz.textContent = unrz.getAttribute("data-unrz");

      this.lang = this.getParam('lang') === 'en' ? 'en' : 'ru';
      ...
      var internationl = document.querySelector('.internationl');
      if (typeof(internationl) != 'undefined' && internationl != null) {
        if (this.lang == 'ru') {
          internationl.classList.add('hide');
        } else {
          internationl.classList.remove('hide');
        }
      }
